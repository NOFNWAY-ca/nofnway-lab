...
// Alarm codes are 6 lowercase alphanumeric chars generated by the client.
// Reject anything that doesn't match to block path traversal and junk keys.
...
export async function onRequestDelete({ params, env, request }) {
    const code = params.code?.[0];
    const op   = params.code?.[1];
    if (!validCode(code) || op !== 'sub') return json({ error: 'Not found' }, 404);
...
async function handleSubscribe(code, env, request) {
    ...
    // SSRF guard: endpoint must be a valid HTTPS URL
    try {
    ...
    if (updated.length >= 20) return json({ error: 'Too many subscribers' }, 429);
